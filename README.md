# 🐿️ FlashSquirrel (閃電松鼠) - Industrial V5 Reborn
> **"丟手機就好" (Drop & Forget)** —— 專為懶人打造的 AI 自動化研究管道。
> **跨平台支援**: ✅ macOS (Native) | ✅ Windows (Beta)
...
### 🛡️ 穩定性版本 | Stability Version: v5.0-reborn (Industrial)
- **[NEW] 跨平台核心 (Cross-Platform Core)**: 原生支援 Windows Task Scheduler 與 MacOS LaunchAgent。
- **[NEW] 智慧清理 (Smart Cleanup)**: 自動辨識並移除 <1KB 的無效垃圾報告，大幅節省儲存空間。
- **[NEW] 隱私清洗 (PII Sanitization)**: 代碼庫移除所有絕對路徑，確保使用者隱私安全。
...
> [!IMPORTANT]
> **本版本處於「智慧清理模式 (Smart Cleanup Mode)」**：為了保持您的資料夾整潔，系統會自動移除**無效的空報告 (<1KB)**。但請放心，系統**絕對不會**幫您刪除任何**原始檔案** (Original Files)。
