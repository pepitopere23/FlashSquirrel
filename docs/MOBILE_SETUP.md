...
### 設定步驟：
1.  **建立捷徑**：[點此下載「閃電松鼠」專用捷徑](https://www.icloud.com/shortcuts/b7238297c2494f73addcd1b7330bdebf)。
2.  **關鍵匹配 (重要)**：
    - 確保您的 iCloud Drive 中，主資料夾名稱必須與腳本設定一致（預設為 **`研究工作流`**）。
    - **進階用法**：您可以直接將檔案存入 `研究工作流/` 根目錄（系統會自動歸類為 "General"），或是存入 `input_thoughts/特定主題/`。
3.  **儲存檔案**：將「分享表單 (Share Sheet)」傳入的內容存入上述路徑。
3.  **使用方式**：
